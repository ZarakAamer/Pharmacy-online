...
class UserLoginView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if Profile.objects.filter(user=user).first():
            ...
            user_cart = CartOrder.objects.filter(user=user_for_data)
            user_orders = Order.objects.filter(user=user_for_data)
            user_data = UserDetailsSerializers(user_for_data, many=False)
            user_carts = CartOrderSerializer(user_cart, many=True)
            user_orders = OrderSerializer(user_orders, many=True)
...
class ProductsViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Product.objects.filter(expiry_date__gte=current_date)
...
class CartOrderProductViewSet(ModelViewSet):
    serializer_class = CartOrderProductEditibleSerializer

    def get_queryset(self):
        return CartOrderProduct.objects.filter(cart__user=self.request.user)
